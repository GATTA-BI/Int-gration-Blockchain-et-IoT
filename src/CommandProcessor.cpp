...
void CommandProcessor::processLedCmd(String cmd){
  ...
  if(cmd.equalsIgnoreCase("clign-rap")){ 
    Serial.println("Received command to clign-rap");
    xTaskCreate(toggleLed, "ToggleFastLED", 1000, (void*)&FAST_LED, 1, &ledTaskHandle);
  } else if(cmd.equalsIgnoreCase("clign-eteint")) {
    Serial.println("Received command to clign-eteint");
  ...
  } else if(!cmd.equalsIgnoreCase("allume")) {
